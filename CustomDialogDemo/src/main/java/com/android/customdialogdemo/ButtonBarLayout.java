...
public class ButtonBarLayout extends LinearLayout {
    ...
    private void setStacked(boolean stacked) {
        ...
        setGravity(stacked ? Gravity.CENTER_HORIZONTAL : Gravity.END);

        final Button button1 = findViewById(R.id.button1);
        final Button button2 = findViewById(R.id.button2);
        final Button button3 = findViewById(R.id.button3);
        setButtonParams(stacked, button1);
        setButtonParams(stacked, button2);
        setButtonParams(stacked, button3);

        final View spacer = findViewById(R.id.spacer);
        if (spacer != null) {
            spacer.setVisibility(stacked ? View.GONE : View.GONE);
        }
    ...
    private void setButtonParams(boolean stacked, Button button) {
        final LayoutParams params = (LayoutParams) button.getLayoutParams();
        if (stacked) {
            params.width = LayoutParams.MATCH_PARENT;
            float scale = getResources().getDisplayMetrics().density;
            int margin = (int) (30 * scale + 0.5f);
